...
void attackTypeDisplay(){
              if (attackType == 1) {
              display.drawBitmap(0, 0, Handler_icon, 128, 64, SH110X_WHITE);
            } else if (attackType == 2) {
                display.drawBitmap(0, 0, EvilTwin_icon, 128, 64, SH110X_WHITE);
              } else if (attackType == 3) {
                display.drawBitmap(0, 0, Deauth_icon, 128, 64, SH110X_WHITE);
                }else{
                  display.drawBitmap(0, 0, earthlink_icon, 128, 64, SH110X_WHITE);
                }
            display.display();
            display.clearDisplay();
}
...
void pageSelectProcess(){
  ...
  int buttonRightState = digitalRead(buttonRight);
          ...
          if(!hotspot_active){
          if (pageDesign == 1) {
              display.drawBitmap(0, 0, Custom_icon, 128, 64, SH110X_WHITE);
            } else {
              if (pageDesign == 2) {
                display.drawBitmap(0, 0, TpLink_icon, 128, 64, SH110X_WHITE);
              } else {
                display.drawBitmap(0, 0, Simple_icon, 128, 64, SH110X_WHITE);
              }
            }
          }
            if(canReturn){
...
void selectNetwork(){
  ...
  if (buttonSelectState == LOW) {
          if(networkPosition != networksNumber){
            ...
            isScaned = false;
            display.drawBitmap(0, 0, scanning, 128, 64, SH110X_WHITE);
            display.display();
            display.clearDisplay();
            performScan();
          }
...
void setup() {
  ...
  display.clearDisplay();
  display.drawBitmap(0, 0, kizohiLogo, 128, 64, SH110X_WHITE);
  display.display();
  display.clearDisplay();
  performScan();
  display.drawBitmap(0, 0, nethunter_icon, 128, 64, SH110X_WHITE);
  display.display();
  display.clearDisplay();
  delay(1000);
  display.setTextSize(2);
